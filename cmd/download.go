...
func filesToDownload(ctx *cmdutil.Ctx) (map[string]file.Op, error) {
	...
	if len(ctx.Args) <= 0 {
		for _, asset := range assets {
			localAsset, _ := shopify.ReadAsset(ctx.Env, asset.Key)
			if asset.Checksum == localAsset.Checksum {
				fetchableFiles[asset.Key] = file.Skip
			} else {
				fetchableFiles[asset.Key] = file.Get
			}
		}
